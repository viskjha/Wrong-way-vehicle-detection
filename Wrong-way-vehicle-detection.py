...
def cvDrawBoxes(detections, img):
    ...
    """
    #================================================================
    # 3.1 Purpose : Filter out vehicle class from detections and get 
    #           bounding box centroid for each vehicle detection.
    #================================================================
    if len(detections) > 0:  						# At least 1 detection in the image and check detection presence in a frame  
        centroid_dict = dict() 						# Function creates a dictionary and calls it centroid_dict
        objectId = 0								# We inialize a variable called ObjectId and set it to 0
        for label, confidence, bbox in detections:				# In this if statement, we filter all the detections for vehicle only
            # Check for the only vehicle name tag 
            name_tag = label   # Coco file has string of all the names
            if name_tag == 'car' or name_tag == 'bus' or name_tag == 'truck':                
                x, y, w, h = (bbox[0],
                            bbox[1],
                            bbox[2],
                            bbox[3])      	# Store the center points of the detections
                ...
                # Append center point of bbox for vehicle detected.
    ...
    #=================================================================
    # 3.2 Purpose : Determine which vehicle bbox are close to each other
    #=================================================================            	
        ...
        cordinate_y = []
        flag = 1
        for id1, p1 in centroid_dict.items():
            # dx, dy = p1[0] - 0, p1[1] - 2500
            # distance = is_close(dx, dy)
            # print(p1[0])
            # print(p1[1])
            # cordinate_x.append(p1[0]-75)
            cordinate_y.append(p1[1]-75)
            if (p1[0] > 250 and p1[0] < 395 and p1[1] > 70 and p1[1] < 500):
        ...
        pre_cordinate_y = []
        if(flag == 1):
            # res = cordinate_y - pre_cordinate_y
            for i,j in zip(cordinate_y,pre_cordinate_y):
                distance = i-j
                # print(distance)
                if(distance < 0):
                    red_line_list.append(id)
                # distance = abs(x-75)
                # pre_cordinate_y.append(distance)
                # print(distance)

        pre_cordinate_x = cordinate_x
        pre_cordinate_y = cordinate_y
        ...
        #     if distance < 60.0:						# Set our social distance threshold - If they meet this condition then..
        #         if id1 not in red_zone_list:
        #             red_zone_list.append(id1)       #  Add Id to a list
        ...
        for idx, box in centroid_dict.items():  # dict (1(key):red(value), 2 blue)  idx - key  box - value
            cv2.line(img, (0, 70), (2500, 70), (255, 0, 0), 4)
            # cv2.rectangle(img,(250, 70) ,(395, 500), (255, 0, 0), 2)
            ...
            if idx in red_zone_list:   # if id is in red zone list
                ...
                # textw = "Wrong way %s " % str(confidence[0:4])
                textw = "Wrong [%s]" % str(label)
                cv2.putText(img, textw, (box[2], box[3]), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            else:
                cv2.rectangle(img, (box[2], box[3]), (box[4], box[5]), (0, 255, 0), 2) # Create Green bounding boxes
                textr = "Correct [%s]" % str(label)
                cv2.putText(img, textr, (box[2], box[3]), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    	...
    	#=================================================================        
        text = "Wrong way: %s " % str(len(red_zone_list)) 			# Count People at Risk
        location = (20,50)												# Set the location of the displayed text
...
def YOLO():
    ...
    out = cv2.VideoWriter(
            "./demo/traffic_output.avi", cv2.VideoWriter_fourcc(*"MJPG"), 10.0,
